    ...
    <v-container class="boxed__container">
      <v-row>
        ...
        <v-col cols="12" md="3">
          ...
          <div class="mt-8">
            <v-range-slider max="500000" min="20000" color="secondary" track-color="grey"
              v-model="price_range"></v-range-slider>

            <div id="price__range" class="d-flex justify-space-between">
              <div class="d-flex">
                <v-text-field :value="price_range[0]" @change="$set(price_range, 0, $event)" outlined flat
                  hide-details="auto" />
                <v-text-field :value="price_range[1]" @change="$set(price_range, 1, $event)" outlined flat
                  hide-details="auto" class="ml-3" />
              </div>

              <v-btn @click="filter_by_price" :loading="btn_loading" depressed color="primary" height="38" width="100"
                class="secondary--text font__12 font-weight-bold">FILTER</v-btn>
            </div>
          </div>
